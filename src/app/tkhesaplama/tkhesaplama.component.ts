...
@Component({
  ...
  templateUrl: './tkhesaplama.component.html',
  styleUrls: ['./tkhesaplama.component.scss'],
})
export class TKHesaplamaComponent implements OnInit {
  ...
  itfa():any{
    while(this.c <= this.vade) {
      let faiztutarı = Math.round(this.tutar *  0.02);
      let KKDF = Math.round(faiztutarı * 0.15);
      let BSMV = Math.round(faiztutarı * 0.05);
